...
|---|---|---|---|---|
| 貼付（入力欄にペースト） | MARK 0122 付近で打ち切り | — | — | 上限 **約 120K 文字**。Copilot 自身が「投稿本文がそのままコンテキストに含まれていた（目視で追った）」と回答 = この範囲は本文がコンテキストに直接入る |
...
- 貼付経路は約 120K 文字までなら全文がコンテキストに入る。120K を超えると末尾から切れるため、末尾 `<instruction>` の欠落はこれでも起きる（先頭リマインダの併用が効く根拠）
- したがってフェーズ 4 の目的は「実業務パックを約 120K 文字以内に収め、貼付経路で全文をコンテキストに入れる」に定まる。要否は実業務パックの文字数が 120K を超えるかで決める
